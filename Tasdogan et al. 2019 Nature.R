...
head(OutputPercentageDF)

df.fra<-gather(OutputPercentageDF,Sample.Raw.File.Name,Percent,-Compound,-Label)
df.fra$Sample.Raw.File.Name<-as.character(df.fra$Sample.Raw.File.Name)
df.fra$Percent[is.nan(df.fra$Percent)]<-0
df.fra$Label<-sub("^","M",df.fra$Label)
MetaboliteList$Compound<-as.character(MetaboliteList$Compound)
df.fra$Compound<-as.character(df.fra$Compound)
df.fra<-left_join(df.fra,MetaboliteList, by = "Compound")

#get sample info from file names
SampleNames<-data.frame(unique(RawData$Sample.Raw.File.Name))
colnames(SampleNames)<-"Sample.Raw.File.Name"
SampleNames<-separate(SampleNames,Sample.Raw.File.Name,c("AA","Genotype","Clone","Labeling.Time"),remove = F)
SampleNames<-unite(SampleNames,Name,Genotype,Labeling.Time,remove = F)
class(SampleNames$Sample.Raw.File.Name)
SampleNames$Sample.Raw.File.Name<-as.character(SampleNames$Sample.Raw.File.Name)
df.fra<-left_join(df.fra,SampleNames,by="Sample.Raw.File.Name")
head(df.fra)


df.fra.grp<-dplyr::group_by(df.fra,Metabolite,Name,Label)
se<-function(x) sd(x)/sqrt(length(x))
df.fra.sum<-dplyr::summarize(df.fra.grp,mean=mean(Percent),sd=sd(Percent),se=se(Percent))

#write it out to define factor order
SampleGroups<-as.character(unique(df.fra.sum$Name))
SampleGroups
class(SampleGroups)

df.fra.sum$Name<-factor(df.fra.sum$Name, levels = 
                          c( "WT_6hrs" , "WT_24hrs" , "Mut_6hrs", "Mut_24hrs"    ))

ggplot(data=df.fra.sum,aes(x=Label,y=mean,fill=Name))+
  ...
  labs(title="All metabolites\n Label:")+
  scale_y_continuous(labels = scales::percent)
ggsave(paste0("Figures all isotopomer",".jpg"), plot = last_plot(),
       width=10,height=7.5)

ggplot(data=df.fra.sum,aes(x=Name,y=mean,fill=Label))+
  geom_bar(stat="identity",position="dodge")+
  facet_wrap(~Metabolite)+
  theme_bw()+
  ...
  labs(title="All metabolites\n Label:")+
  scale_y_continuous(labels = scales::percent)
ggsave(paste0("Figures all group",".jpg"), plot = last_plot(),
       width=10,height=7.5)
